...
function transfer_logtype_call() {
    moduleConfig=(audio common power wifi)
    transferModules
}
...
function transfer_logtype_wifi() {
    moduleConfig=(common performance power stability thirdpart wifi)
    transferModules
}
...
function collect_function_common_persistLog() {
    ...
    checkNumberSizeAndCopy "${DATA_OPLUS_LOG_PATH}/backup" "${TARGET_DATA_OPLUS_LOG}/backup" "1024000"
    #no need Obrain
    rm -rf "${TARGET_DATA_OPLUS_LOG}/backup/OTRTA/manually_traces/Obrain"
