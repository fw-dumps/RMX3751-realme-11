...
CELL_EG as (
    ...
    from (
        select json_object('mode', ground_mode || screen_mode || thermal_mode || power_mode
        , 'ntyp', networkType, 'dmod', duplexMode
        , 'sim', simState, 'ban', bands, 'in_du', sum(incall_du)
        , 'rbyt', sum(rx_trans_byte), 'tbyt', sum(tx_trans_byte)
        ...
        , 't4_tm', round(SumVectorI('sum(lvl$0_duration)', CellularTxLevelDurationList), 2)
        , 't5_tm', round(SumVectorI('sum(lvl$0_5g_duration)', CellularTx5gLevelDurationList), 2)
        , 'slp_tm', round(sum(sleep_time), 2), 'r_tm', round(sum(rx_time), 2)
        , 'r5_tm', round(sum(rx_5g_time), 2), 'tr_eg', sum(transceiver_eg)
        , 'pa_eg', sum(pa_eg), 'md_eg',  sum(modem_eg), 'tt_eg', sum(total_eg)) as cell_energy
...
TOP_APP_BY_MODE as (
    ...
    from (
        select json_object('app', T1.uid, 'name', obfuscate(T1.name)
        , 'mode', ground_mode || screen_mode || thermal_mode || power_mode
        , 'ntyp', networkType, 'dmod', duplexMode
        , 'sim', simState, 'ban', bands, 'r_byt', sum(rx_trans_byte), 't_byt', sum(tx_trans_byte)
        , '4g_du', json_array(VectorI('round(sum(lvl$0_duration), 2)', CellularTxLevelDurationList))
        , '5g_du', json_array(VectorI('round(sum(lvl$0_5g_duration), 2)', CellularTx5gLevelDurationList))
        , 't4_tm', round(SumVectorI('sum(lvl$0_duration)', CellularTxLevelDurationList), 2)
        , 't5_tm', round(SumVectorI('sum(lvl$0_5g_duration)', CellularTx5gLevelDurationList), 2)
        , 'slp_tm', round(sum(sleep_time), 2), 'r_tm', round(sum(rx_time), 2)
        , 'r5_tm', round(sum(rx_5g_time), 2), 'tr_eg', sum(transceiver_eg)
        , 'pa_eg', sum(pa_eg), 'md_eg',  sum(modem_eg), 'tt_eg', sum(total_eg)) as top_app_by_mode
        from (
            select *
...
SIGNAL_STATS as (
    ...
    from (
        select json_object('lv', signalLevel, 'du', sum(total_du)) as by_signal_level
